...
public class SplitSecondStopwatch {
    ...
    public void advanceTime(String timeString) {
        if(this.state.equals("running")){
            this.currentLap = this.currentLap.plusSeconds(LocalTime.parse(timeString).toSecondOfDay());
            this.total = this.total.plusSeconds(LocalTime.parse(timeString).toSecondOfDay());
        }
    }
}